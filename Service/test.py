...
urllibOutput = urlopen(input, context=gcontext).read().decode("utf-8")
...
rawOutput = re.findall('<a href="/wiki(.*?)</a>', urllibOutputUpToReferences[0])
parsedOutput = {}
...
parsedOutput["title"] = re.search("https://en.wikipedia.org/wiki/(.*)", input).group(1)
parsedOutput["listOfSublinks"] = []

for r in rawOutput:
    try:
        rLink = "https://en.wikipedia.org/wiki" + re.search('(.*?)"', r).group(1)
