import wikipedia, pika, string, uuid, sys
...
def get_top_words(page_name, num_words, incl_links):
    ...
    ordered_words = [
        freqs[0]
        for freqs in sorted(word_freqs.items(), key=lambda item: item[1], reverse=True)
    ]
...
def respond_to_RabbitMQ():
    connection = pika.BlockingConnection(
        pika.URLParameters("amqp://leoparduser:E6dXz8t!fPthwfr@40.71.6.249")
    )
    ...
    channel.basic_consume(queue="topwords_queue", on_message_callback=on_request)
...
if sys.argv[3] == "True":
    ...
    print(usage)
    sys.exit()
myString = get_top_words_str(wikipedia_page_name, num_top_words, neighboring_entries)
print(myString)
